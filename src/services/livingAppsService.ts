...
import type { BenutzerRollen, CreateBenutzerRollen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getBenutzerRollen(): Promise<BenutzerRollen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BENUTZER_ROLLEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as BenutzerRollen[];
    return enrichLookupFields(records, 'benutzer_&_rollen');
  }
  static async getBenutzerRollenEntry(id: string): Promise<BenutzerRollen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BENUTZER_ROLLEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as BenutzerRollen;
    return enrichLookupFields([record], 'benutzer_&_rollen')[0];
  }
  static async createBenutzerRollenEntry(fields: CreateBenutzerRollen) {
    return callApi('POST', `/apps/${APP_IDS.BENUTZER_ROLLEN}/records`, { fields: cleanFieldsForApi(fields as any, 'benutzer_&_rollen') });
  }
  static async updateBenutzerRollenEntry(id: string, fields: Partial<CreateBenutzerRollen>) {
    return callApi('PATCH', `/apps/${APP_IDS.BENUTZER_ROLLEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'benutzer_&_rollen') });
  }
  static async deleteBenutzerRollenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BENUTZER_ROLLEN}/records/${id}`);
  }

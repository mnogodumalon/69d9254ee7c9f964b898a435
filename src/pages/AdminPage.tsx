...
const BENUTZERROLLEN_FIELDS = [
  ...
  { key: 'rolle', label: 'Rolle im System', type: 'lookup/select', options: [{ key: 'facilitator', label: 'Facilitator' }, { key: 'curator', label: 'Curator' }, { key: 'ai_support', label: 'AI-Support' }, { key: 'admin', label: 'Admin' }, { key: 'viewer', label: 'Viewer' }, { key: 'contributor', label: 'Contributor' }] },
  { key: 'abteilung', label: 'Abteilung / Organisationskontext', type: 'string/text' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
  { key: 'aktiv', label: 'Aktiv im System', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'benutzer_&_rollen', label: 'Benutzer & Rollen', pascal: 'BenutzerRollen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('benutzer_&_rollen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'benutzer_&_rollen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'benutzer_&_rollen': return (data as any).benutzerRollen as BenutzerRollen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'benutzer_&_rollen': return BENUTZERROLLEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'benutzer_&_rollen' || dialogState?.entity === 'benutzer_&_rollen') && (
        <BenutzerRollenDialog
          open={createEntity === 'benutzer_&_rollen' || dialogState?.entity === 'benutzer_&_rollen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'benutzer_&_rollen' ? handleUpdate : (fields: any) => handleCreate('benutzer_&_rollen', fields)}
          defaultValues={dialogState?.entity === 'benutzer_&_rollen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['BenutzerRollen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['BenutzerRollen']}
        />
      )}
      {viewState?.entity === 'benutzer_&_rollen' && (
        <BenutzerRollenViewDialog
          open={viewState?.entity === 'benutzer_&_rollen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'benutzer_&_rollen', record: r }); }}
        />
      )}

...
export default function BenutzerRollenPage() {
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Benutzer & Rollen in the system`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Search Benutzer & Rollen..."
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('abteilung')}>
                <span className="inline-flex items-center gap-1">
                  Abteilung / Organisationskontext
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.aktiv ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.aktiv ? 'Yes' : 'No'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Benutzer & Rollen yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
